...
function App() {
  ...
  // States
  const [algo, setAlgo] = useState("bubbleSort"); // which algo is selected right now
  const [len, setLength] = useState(5); // what is the len of our current array
  ...
  const handleSpeed = (event) => {
    let Speed = 400 / Number(event.target.value);
